...
public:
    ...
    static bool isCrossRedundant(const EdgePair &cross);
    EdgePairContainer safeRemoveCrossingEdgesAndReturnUnsafe();
...
bool PixelGraph::isCrossRedundant(const PixelGraph::EdgePair &cross)
{
    /* A | B
       --+--
       C | D */
    PixelGraph::iterator a = cross.first.first;
    PixelGraph::iterator b = cross.second.first;
    PixelGraph::iterator c = cross.second.second;
    //PixelGraph::iterator d = cross.first.second;

    return a->adj.right && a->adj.bottom && b->adj.bottom && c->adj.right;
}

PixelGraph::EdgePairContainer
PixelGraph::safeRemoveCrossingEdgesAndReturnUnsafe()
{
    ...
    for (int i = 0 ; i != height() - 1 ; ++i, ++it ) {
        for ( int j = 0 ; j != width() - 1 ; ++j, ++it ) {
            ...
            if ( same_color(diagonals.second.first->rgba,
                            diagonals.second.second->rgba)
                 && !same_color(diagonals.first.first->rgba,
                                diagonals.first.second->rgba) ) {
                // main diagonal
                diagonals.first.first->adj.bottomright = 0;
                diagonals.first.second->adj.topleft = 0;
            } else if ( same_color(diagonals.first.first->rgba,
                                   diagonals.first.second->rgba)
                        && !same_color(diagonals.second.first->rgba,
                                       diagonals.second.second->rgba) ) {
                // secondary diagonal
                diagonals.second.first->adj.bottomleft = 0;
                diagonals.second.second->adj.topright = 0;
            } else if (isCrossRedundant(diagonals)) {
                // main diagonal
                diagonals.first.first->adj.bottomright = 0;
                diagonals.first.second->adj.topleft = 0;
