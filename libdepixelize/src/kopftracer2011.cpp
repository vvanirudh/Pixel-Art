                   ...
                   const Options &options)
{
...

    {
        ...
        // the data being read.
        PixelGraph::EdgePairContainer edges
            = graph.safeRemoveCrossingEdgesAndReturnUnsafe();

#ifdef LIBDEPIXELIZE_PROFILE_KOPF2011
    profiling_info[1] = Glib::DateTime::create_now_utc();
    std::cerr
        << "Tracer::PixelGraph::safeRemoveCrossingEdgesAndReturnUnsafe() time: "
        << profiling_info[1].difference(profiling_info[0]) << std::endl;
#endif // LIBDEPIXELIZE_PROFILE_KOPF2011
    ...
    assert(graph.safeRemoveCrossingEdgesAndReturnUnsafe().size() == 0);
