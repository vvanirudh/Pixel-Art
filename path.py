...
def slope(p0, p1):
	...
	dy = p1[1] - p0[1]
	if dx == 0:
		return dy * 99999999999999 # Since dx is zero, we avoid division by zero. Instead, we multiply dy by a large number
	return 1.0 * dy / dx
...
class Path(object):
	...
	def make_path(self, shape_graph):
		...
		neighbors = sorted(shape_graph.neighbors(path[0]),
						   key=lambda p: slope(path[0], p))
		path.append(neighbors[0])
		nodes.difference_update(path) # This is equivalent to set difference between nodes and path
	...
	def make_spline(self):
		...
		self.spline = bspline.Curve2Closed_BSpline(self.path)

	def smooth_spline(self):
		# Optimise the spline that is already fit to the path
		self.smooth = smoothen.smooth_spline(self.spline)
