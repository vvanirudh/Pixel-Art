...
class BSpline(object):
	...
	def __init__(self, KnotVector, points, degree=None):
		...
		expected_degree = len(self.KnotVector) - len(self._points) - 1 # Expected degree, if not specified in the constructor
		if degree is None:
			degree = expected_degree
		if degree != expected_degree:
			raise ValueError("Degree expected is %s, got %s instead as Input." % (expected_degree, degree))
		self.degree = degree
		self.remove_stored() 
	...
	def __call__(self, u):
		...
		ps = [dict(zip(range(k - self.degree, k - s + 1), self.points[k - self.degree:k - s + 1]))]

		for r in range(1, self.degree - s + 1):
			ps.append({})
			for i in range(k - self.degree + r, k - s + 1):
				a = (u - self.KnotVector[i]) / (self.KnotVector[i + self.degree - r + 1] - self.KnotVector[i])
	...
	#	Finding a series of quadratic Beziers making up this spline.
		...
		for cp, ocp1 in zip(control_points, on_curve_points[1:]):
			yield (ocp0.tuple, cp.tuple, ocp1.tuple) # Yield the starting point of the curve, control point and end point
			ocp0 = ocp1
...
class Closed_BSpline(BSpline):
	...
	def Span(self, index):
		span = lambda i: (self.KnotVector[i], self.KnotVector[i + 1])
		d0, d1 = span(index)
		if d0 < self.domain[0]:
