...
def check_bounds(coordinate, size, offset=(0, 0)):
	x, y = map(sum, zip(coordinate, offset)) # Add the coordinates of the given coordinate to the offset
	width, height = size
	return (0 <= x < width and 0 <= y < height) # If the coordinates lie outside the dimensions of the image then return false


class Heuristics(object):
	sparse_window = (8, 8) # The window around the edge which we examine for sparse heuristic

	def __init__(self, pixel_graph): # The pixel data of the image is loaded into the heuristic
		self.pixel_graph = pixel_graph

	def sparse_offset(self, edge): # This returns the boundaries of the window that must be examined in the sparse heuristic
		return (
	...
	def apply_heuristics(self, ambiguous_diagonal_pairs):
		...
		for edges in ambiguous_diagonal_pairs:
			self.weight_diagonals(*edges) # Evaluate the heuristic value on each diagonal

		for edges in ambiguous_diagonal_pairs:
			min_weight = min(e[2]['weight'] for e in edges)
			for edge in edges:
				if edge[2]['weight'] == min_weight: 
					self.pixel_graph.remove_edge(*edge[:2]) # Remove the edge which has minimum weight among the pair
				else:
					edge[2].pop('weight') # If it is not the diagonal removed, then the attribute weight is removed
	...
	def weight_curve(self, edge):
		...
		while nodes_in_the_curve:
			...
			if len(edges) != 2: # If the node is not a valence-2 node then it can't be part of a curve
				continue
			...
			for edge in edges:
				...
				if edge not in edges_in_the_curve: # If edge not already seen
					...
					nodes_in_the_curve.extend(n for n in edge if n != node) # Add the new nodes to the seen nodes
	...
	def weight_sparse(self, edge):
		...
		return -len(nodes_in_the_sparse_window) # Returns the negative of the nodes found
